<template>
    ...
    >
        <div class="vdr-body" :style="rotateStyle">
            <div
            ...
            <i
                ...
                class="vdr-rotate-icon iconfont iconxuanzhuan1"
                @mousedown.stop.prevent="handleRoateDown"
                @touchstart.stop.prevent="handleRoateTouchDown"
            ></i>
            <slot></slot>
        </div>
    </div>
...
import { getOffsetTop, getOffsetLeft } from '../tools'
import { calculateAngleDegree } from '../tools/attach-tools'
...
export default {
    ...
    methods: {
        ...
        handleDown(handle, e) {
            ...
            this.actualClickedHandle = handle
        ...
        handleRoateTouchDown(e) {
            eventsFor = events.touch

            this.handleRoateDown(e)
        },
        handleRoateDown() {
        ...
        elementMove(e) {
            ...
            this.snapCheck() // 原始辅助线处理函数
            // this.snapContainerCheck()
        ...
        snapToGrid(grid, pendingX, pendingY) {
            ...
            return [
                Math.floor(x / this.scaleRatio),
                Math.floor(y / this.scaleRatio)
            ]
        },
        // 新增方法 ↓↓↓
        // 设置属性
            ...
            if (this.snap) {
                ...
                for (let item of nodes) {
                    if (
                        ...
                        const ts =
                            Math.abs(t - activeBottom) <= this.snapTolerance // 从上到下
                        const TS =
                            Math.abs(b - activeBottom) <= this.snapTolerance // 从上到下
                        const bs = Math.abs(t - activeTop) <= this.snapTolerance // 从下到上
                        const BS = Math.abs(b - activeTop) <= this.snapTolerance // 从下到上

                        const ls =
                            Math.abs(l - activeRight) <= this.snapTolerance // 外左
                        const LS =
                            Math.abs(r - activeRight) <= this.snapTolerance // 外左
                        const rs =
                            Math.abs(l - activeLeft) <= this.snapTolerance // 外右
                        const RS =
                            Math.abs(r - activeLeft) <= this.snapTolerance // 外右
                        ...
                        if (ts) {
                            if (bln) {
                                ...
                                this.rawBottom =
                                    this.parentHeight - this.rawTop - height
                            }
                        ...
                        if (ls) {
                            if (bln) {
                                ...
                                this.rawRight =
                                    this.parentWidth - this.rawLeft - width
                            }
        ...
        async snapContainerCheck() {
            ...
            if (this.snap) {
                ...
                const l = 0 // 对齐目标的left
                const r = this.parentWidth // 对齐目标right
                const t = 0 // 对齐目标的top
                const b = this.parentHeight // 对齐目标的bottom
                ...
                for (let i = 0; i <= arrTem.length; i++) {
                    ...
                    const horv = i < 6 ? 'hLine' : 'vLine'
                    if (tem.display[i]) {
                        const { origin } = this.calcLineValues(
                            tem.value[xory][arrTem[i]]
                        )
                        ...
                        refLine[horv][arrTem[i]].position =
                            tem.position[i] + 'px'
                        refLine[horv][arrTem[i]].origin = origin
                        refLine[horv][arrTem[i]].lineLength =
                            i < 6
                                ? this.parentWidth + 'px'
    ...
    computed: {
        style() {
            return {
                position: 'absolute',
                transform: `translate( ${this.left + 'px'} , ${this.top +
                    'px'} )`,
        ...
        isCornerHandle() {
            return (
                Boolean(this.actualClickedHandle) &&
                ['tl', 'tr', 'br', 'bl'].includes(this.actualClickedHandle)
            )
        }
    },
    ...
    watch: {
        ...
        rawLeft(newLeft) {
            ...
            if (lockAspectRatio && this.resizingOnX && this.isCornerHandle) {
        ...
        rawTop(newTop) {
            ...
            if (lockAspectRatio && this.resizingOnY && this.isCornerHandle) {
...
.vdr.active .vdr-body::before,
.vdr:hover .vdr-body::before {
    position: absolute;
    display: block;
    content: '';
    border: 1px dashed @attach-color;
    height: 100%;
    width: 100%;
    margin: -1px;
    z-index: 99;
